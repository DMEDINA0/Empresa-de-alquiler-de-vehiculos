...
class Factura(Base):
    ...
    id_factura = Column(UUID(as_uuid=True), primary_key=True, default=uuid4, unique=True, nullable=False)
    fecha_emision = Column(DateTime, nullable=False)
    monto_total = Column(Integer, nullable=False)
    id_alquiler = Column(UUID(as_uuid=True), ForeignKey("alquiler.id_alquiler"), nullable=False)
